...
colour = ['blue','black','silver','red','darkorange','gold','darkorchid','aqua','cadetblue','darkkhaki','pink','moccasin','cornflowerblue','chartreuse','limegreen','darkgreen','chocolate','darkgoldenrod']
...
for i in range(4,Ncol3):
  ...
  if (np.max(Natmos)>Nmax*1.E-10):
    lab = keyword3[i]
    plt.plot(np.log10(t[iii]/yr),np.log10(Natmos[iii]),label=lab,c=colour[col],lw=3)
    col = col+1
...
for i in range(4,Ncol1):
  ...
  if (np.max(Nsolid)>Nmax*1.E-5):
    ...
    lab = keyword1[i]
    plt.plot(np.log10(t[iii]/yr),np.log10(Nsolid[iii]),label=lab,c=colour[col],lw=3)
    col = col+1
