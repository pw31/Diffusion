...
************************************************************************
      ...
      real*16,parameter :: span=1.Q-3
      real*16 :: n1,n2,a1,a2
      real*16 :: aa,bb,cc,pp,qq,VIETA1,VIETA2,VIETA3,VIETA4
      integer :: fail,it
      logical :: ok,raus=.false.
      character(len=2) :: bem
*-----------------------------------------------------------------------
      VIETA1(pp,qq) = qq/(-pp/2.Q0-DSQRT(pp**2/4.Q0-qq))
      VIETA2(pp,qq) = qq/(-pp/2.Q0+DSQRT(pp**2/4.Q0-qq))
      VIETA3(pp,qq) = -pp/2.Q0+DSQRT(pp**2/4.Q0-qq)
      VIETA4(pp,qq) = -pp/2.Q0-DSQRT(pp**2/4.Q0-qq)
*-----------------------------------------------------------------------
      ...
      if (ok) then
        ...
        ok = (a1>0.Q0.and.(L1.ne.a1*L0))
        fail = 2
      endif

      if (ok) then
        a2 = (L2-a1*L1)/(L1-a1*L0)
        if ((a2<span*a1).or.(a1==a2)) then
          !print*,a1,a2 
          do it=1,10
            a2 = span*a1
            !aa = L1-a2*L0
            ...
            if ((.not.ok).or.(span*a1<a2)) exit
          enddo  
          ok = ok.and.(a1>span*a2)
          fail = 3
          bem = ' 3'
        endif  
      endif
