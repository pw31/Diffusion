...
************************************************************************
     ...
     >               dd1l,dd1m,dd1r
      ...
      use PARAMETERS,ONLY: pmin,pmax,logg
      implicit none
      integer :: i,j
      real*8 :: fac,fac1,fac2,hmin,hmax,h1,h2,df,dz,int,norm
      real*8 :: ngas,Hp,lmean
     ...
     >         d2l(-2:N),d2m(-2:N),d2r(-2:N))
      ...
      !---- physical properties ----
      write(*,*)
      write(*,1000) "","z[km]","p[bar]","T[K]","mu[amu]",
     >              "Diff[cm2/s]","lmean/Hp"
      j=Nlayers
      do i=-2,N
        do 
        ...
        Hp       = bk*Temp(i)/(10.0**logg*mu(i))
        ngas     = press(i)/bk/Temp(i)
        lmean    = 1.d0/(2.1E-15*ngas)
        write(*,1010) i,j,(zz(i)-zz(0))/km,press(i)/bar,Temp(i),
     >                mu(i)/amu,Diff(i),lmean/Hp
      enddo
      zz(:) = zz(:)-zz(0)
