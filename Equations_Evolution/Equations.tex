...
to formulate our inner boundary condition as follows
\begin{center}
\begin{tabular}{lcl}
...
with fixed $\ek$ boundary condition, which results in the boundary
element fluxes
\begin{equation}
  \mbox{limiting element $k=1,...,J$:}\quad\quad 
   j_k = -\nH D \frac{d\ek}{dz}\Big|_{z=0} \ .
\end{equation}
Next, we reconstruct the changes of the crust column densities $N_j^{\rm
  cond}$ from the $k=1,...,J$ limiting element boundary fluxes by
solving the following system of linear equation 
\begin{equation}
  \sum_j s_{j,k} \frac{dN_j^{\rm cond}}{dt} ~=~ j_k \ ,
\end{equation}
which are $J$ equations for $J$ unknowns. Finally we compute the
...
of O available, so there is one non-limiting element ($N=1$). Using 
the condition of phase equilibrium at the bottom of the atmosphere 
is assumed to fix the element abundances $\ek$ of the limiting
elements. These values are used as boundary conditions to run 
the diffusion solver for Mg and Si, along with all other elements
not involved in the condensation/outgassing process. As a result,
the element fluxes through the inner boundary $j_{\rm Mg}$ and $j_{\rm
  Si}$ are determined. To reconstruct the changes of column densities.    
